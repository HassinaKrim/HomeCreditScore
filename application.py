...
import re
st.set_page_config(
        page_title='Application scoring dashboard',
        page_icon = "🏡",
        layout="wide"
    )
#st.set_page_config(layout="wide") #le layout est configuré en largeur pour que les éléments de l'application soient affichés sur toute la largeur de la page.
...
st.caption("""
            <p style="color:Black;text-align:center;font-size:1.5em;font-style:italic;font-family:'Roboto Condensed';margin:0px;">
            Hassina KRIM - OpenClassrooms Projet n°7 - Data Scientist</p>
            """, 
            unsafe_allow_html=True)
    
# Description du projet
st.markdown("""
            <p style="color:black;font-family:'Hassina KRIM';">
            <br><br>
...
st.caption("""
            <p style="color:black;font-family:'Hassina KRIM';">
            <strong>En cliquant sur le bouton  "Votre résultat"</strong>, vous pourrez découvrir le score 
            qu'il a obtenu pour sa demande de prêt.
            </p>
            """, unsafe_allow_html=True)
...
def main():
    ...
    def predict(donnees, index): 
        ...
        url ="https://hassinaoc-pszyc.eastus2.inference.ml.azure.com/score"
        
        # Replace this with the primary/secondary key or AMLToken for the endpoint
        api_key = 'zsdnY9KhefbWqg4P4vuCEvl6bxX6rW4M'
        if not api_key:
            raise Exception("A key should be provided to invoke the endpoint")

        # The azureml-model-deployment header will force the request to go to a specific deployment.
        # Remove this header to have the request observe the endpoint traffic rules
        headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key), 'azureml-model-deployment': 'newmodel-1' }
        ...
        try:
            ...
            print("Response body:", result_json)
        ...
        # Convertir la chaîne de caractères encodée en bytes en float
        #score = float(res_str_list[0])
    ...
    df_wide.loc[:, 'BURO_DAYS_CREDIT_MEAN'] = abs(df_wide['BURO_DAYS_CREDIT_MEAN'])
    ...
    df_clients_filtered.loc[:, 'BURO_DAYS_CREDIT_MEAN'] = abs(df_clients_filtered['BURO_DAYS_CREDIT_MEAN'])
